...
if (typeof window.validateStep !== 'function') {
  window.validateStep = (step) => {
    ...
    let activeValidationTooltipTimer = null;

    const hideValidationTooltip = () => {
      if (activeValidationTooltipTimer) {
        window.clearTimeout(activeValidationTooltipTimer);
        activeValidationTooltipTimer = null;
      }

      if (activeValidationControl) {
    ...
    const showValidationFeedback = (control, message, isResolved = () => control.checkValidity()) => {
      ...
      activeValidationTooltipTimer = window.setTimeout(() => {
        if (activeValidationTooltip === tip) {
          hideValidationTooltip();
        }
      }, 1800);
    };
